...
#TODO remove this
sudo docker rmi --force $(sudo docker images -q) && sudo docker rm --force $(sudo docker ps -a -q)
...
# sudo docker exec istoresumido-backend-1 python3 -m RAGeneration.populate_database.py
